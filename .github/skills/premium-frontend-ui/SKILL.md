...
As an AI engineering assistant, your role when building premium frontend experiences goes beyond outputting functional HTML and CSS. You must architect **immersive digital environments**. This skill provides a modular blueprint for generating highly intentional, award-level web applications that prioritize aesthetic quality, deep interactivity, and flawless performance.

When a user requests a high-end landing page, an interactive portfolio, or a specialized component that requires top-tier visual polish, apply the following standards in phases: start with structure, then motion, then visual polish, and always finish with performance and accessibility checks.

---

## 1. Establishing the Creative Foundation

Before generating layout code, ensure you understand the specific emotional tone the UI should evoke in users (for example: energetic, calm, or technical). Do not default to generic, unopinionated code.
